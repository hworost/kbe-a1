...
class RailElement:
	...
	def getPath(self):
		path = None
		if self.type == "line":
			self.end_point = [self.x0+self.length*ma.cos(self.angle+ma.pi/2), self.y0 + self.length*ma.sin(self.angle+ma.pi/2), self.z0]
			self.path = Line(self.x0, self.y0, self.z0, self.end_point[0], self.end_point[1], self.end_point[2])
		if self.type == "arc":
	...
	def get_cross_section_xz(self):
		...
		if(self.type == "line"):
			...
			z0 = self.z0
			a = self.angle
			line1 = Line(x0, y0, z0, x0 + b*ma.cos(a), y0 + b*ma.sin(a), z0)
			line2 = Line(x0 + b*ma.cos(a), y0 + b*ma.sin(a), z0, x0 + b*ma.cos(a), y0 + b*ma.sin(a), z0 + t)
			line3 = Line(x0 + b*ma.cos(a), y0 + b*ma.sin(a), z0 + t, x0 + (b+s)/2*ma.cos(a), y0 + (b+s)/2*ma.sin(a), z0 + t) #3
			...
			line7 = Line(x0 + b*ma.cos(a), y0 + b*ma.sin(a), z0 + h, x0, y0, z0 + h)
